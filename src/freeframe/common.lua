...
-- @Last Modified time	: 2015-01-31 20:03:03
-- @Description			: wapan
--========================================================================
...
function RequireScript()
	...
	for _, func_item in pairs(__init_function_list) do
		...
		if not result or ret_code ~= 1 then
			assert(false, func_item.name.." execute failed")
			return 0
		end
	end
	return 1
...
function ShowTB(table_raw, n)
	...
	local function printTable(table, deepth, max_deepth)
		...
		for k, v in pairs(table) do
			if type(v) ~= "table" then
				...
				printTable(v, deepth + 1, max_deepth)
			end
		end
	end
	printTable(table_raw, 1, n)
end